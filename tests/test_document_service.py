...
from document_service import DocumentProcessingError, extract_document
...
class DocumentServiceTests(unittest.TestCase):
    ...
    def test_pdf_extraction_preserves_page_markers_and_layout(self, reader_mock) -> None:
    ...
    def test_lazy_pdf_parse_failure_has_safe_error(self, reader_mock) -> None:
        class BrokenReader:
            ...
            @property
            def pages(self):
                raise document_service.PdfReadError("broken page tree")
    ...
    def test_pdf_page_limit_is_enforced(self, reader_mock) -> None:
        reader_mock.return_value = SimpleNamespace(
            is_encrypted=False,
            pages=[MagicMock() for _ in range(document_service.MAX_PDF_PAGES + 1)],
        )
    ...
    def test_image_only_pdf_has_actionable_error(self, reader_mock) -> None:
        ...
        reader_mock.return_value = SimpleNamespace(is_encrypted=False, pages=[page])
    ...
    def test_extracted_character_limit_is_enforced(self, reader_mock) -> None:
        ...
        with patch("document_service.MAX_EXTRACTED_CHARACTERS", 20):
            with self.assertRaises(DocumentProcessingError) as context:
    ...
    @patch(
        "document_service.subprocess.run",
